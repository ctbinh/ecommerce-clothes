...
const Checkout = () => {
  ...
  const onPay = () => {
    ...
    axios
      .post("http://localhost/ecommerce/backend/api/order/create.php", data)
      .then((response) => {
    ...
    axios.post("http://localhost/ecommerce/backend/api/cart/deleteAll.php", data_delete, config).then((response) => {
      console.log(response.data);
    });
    navigate("../");
  };
                  ...
                  <Row>
                    <Col md={7}>
                      <ButtonPay onClick={() => onPay()}>
                        Pay ${/* {lstCart.reduce((a, b) => {return a.amount *Number(a.price) + b.amount *Number(b.price)}, 0)} */}
                        {lstCart?.reduce((sum, product) => {
